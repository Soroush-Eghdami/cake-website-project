        ...
        <nav className="flex flex-col mt-4 px-4 gap-0.5 flex-1 overflow-y-auto">
          {navLinks.map((item) => (
            <a
              key={item}
              onClick={onClose}
              className="footer-link px-4 py-3.5 text-base font-medium text-gray-700 hover:text-red-700 rounded-xl hover:bg-bg-[#F5E6E6] transition-colors relative after:absolute after:left-4 after:-bottom-0.5 after:h-0.5 after:w-0 after:bg-[#B83232] after:transition-all after:duration-300 hover:after:w-[calc(100%-2rem)]"
            >
              {item}
            </a>
          ))}
